...
\begin{itemize}
	...
	\item I also found sigma squared or the residual variance of the linear model, so I can compare it to the output of the MLE function.
\end{itemize}
...
\begin{itemize}
	\item The linear.lik function is defined; it takes 3 arguments, theta, y, and x. 
...
\lstinputlisting[language=R, firstline=110,lastline=113]{PS01_KC.R} 
\begin{verbatim}
...
\noindent This suggests that Ordinary Least Squares and Maximum Likelihood will have approximately the same results when used to estimate a linear regression. While OLS is still more useful when our assumptions are met (because it gives an unbiased estimate of variance), MLE can be used in cases when the assumptions for linear regression using OLS are not met.
